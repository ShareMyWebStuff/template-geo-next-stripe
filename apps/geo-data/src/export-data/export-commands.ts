...
export const exportCommandFile = async ( ) => {
    ...
    let sql = `TRUNCATE TABLE import_cities; \n`
    sql += `TRUNCATE TABLE import_partials; \n`
    sql += `TRUNCATE TABLE import_postcodes; \n`
    sql += `TRUNCATE TABLE import_countries; \n`
    sql += `TRUNCATE TABLE import_city_differences; \n`

    // Load the country import table
    sql += `LOAD DATA INFILE '${EXPORT_DIRECTORY}countries.txt'    INTO TABLE import_countries    FIELDS TERMINATED BY '\\t' LINES TERMINATED BY '\\n';\n`

...
export function HeaderNav({ source, manualExpand }: HeaderNavProps) {
  ...
  const underline =
    "py-1 sm:pr-5 decoration-1 underline-offset-4 hover:underline";
          ...
          return (
            <li key={index} className={"group py-1 pr-5"}>
              ...
              <div
                className={
                  "sm:pt-2" +
                  (manualExpand && menuChecked[index] ? " mt-2" : " sm:mt-0")
                }
              >
                <ul
                  className={`group-hover:md:block ${manualExpand && menuChecked[index] ? "block" : "hidden"} sm:absolute static  bg-blue-normal sm:bg-white-800 text-white-800 sm:text-blue-dark  border-solid rounded-lg px-3 sm:p-3 z-[1000]`}
                  >
